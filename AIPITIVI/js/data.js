...
// ── Proxy configuration ──────────────────────────────────
// The Node.js server (server.js) handles the '/proxy' endpoint.
const CLOUDFLIX_PROXY = '/proxy';
...
async function getSeriesInfo(seriesId) {
  return apiGet(XTREAM.api('get_series_info', `&series_id=${seriesId}`));
}

async function getVodInfo(vodId) {
  return apiGet(XTREAM.api('get_vod_info', `&vod_id=${vodId}`));
}
