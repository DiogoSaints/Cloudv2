...
function _proxyApiUrl(rawUrl) {
    // CLOUDFLIX_PROXY defined in data.js (loaded before api.js)
    if (!_apiIsHosted || typeof CLOUDFLIX_PROXY === 'undefined' || !CLOUDFLIX_PROXY) return rawUrl;
    return CLOUDFLIX_PROXY + '?url=' + encodeURIComponent(rawUrl.replace(/^http:\/\//i, 'https://'));
}
