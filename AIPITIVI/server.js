...
// ── Cache for API/JSON only (5 min) ──────────────────────
const cache = new Map();
function cached(k) {
    const e = cache.get(k);
    if (!e) return null;
    if (Date.now() - e.t > 300000) { cache.delete(k); return null; }
    return e;
}
...
app.get('/proxy', async (req, res) => {
    ...
    // ── MP4/VOD Direct Redirect (Offload processing) ──
    if (isVideo) {
        console.log('[proxy] REDIRECT VOD', url.substring(0, 60));
    ...
    try {
        ...
        if (isActuallyStream) {
            console.log('[proxy] PIPE', url.substring(0, 60));

            // Forward relevant headers but EXCLUDE Content-Length to avoid mismatch errors
            if (ct) res.set('Content-Type', ct);
            if (resp.headers.get('content-range')) res.set('Content-Range', resp.headers.get('content-range'));

            // If the upstream was compressed, fetch decompressed it. 
            // We must NOT send the original Content-Encoding or Content-Length.
